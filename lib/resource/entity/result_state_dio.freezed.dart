...
class _$ResultStateTearOff {
  ...
  _Data<T> data<T>({required T data}) {
    return _Data<T>(
      data: data,
    );
  }

  _Error<T> error<T>({required NetworkError error}) {
    return _Error<T>(
      error: error,
    );
  }
...
abstract class _Idle<T> implements ResultState<T> {
...
abstract class _Data<T> implements ResultState<T> {
  const factory _Data({required T data}) = _$_Data<T>;
...
abstract class _Error<T> implements ResultState<T> {
  const factory _Error({required NetworkError error}) = _$_Error<T>;
